(function(){
      ...
      '<div class="wep-card-main">'+
        ...
        '<section class="wep-card-buy">'+
          ...
          '">'+
            ...
            '<a href="https://www.westendpower.com/stihl-equipment-configurator/product-options.html?sku='+
  ...
  function renderSmartMarkup(data,category){
    var seriesPage=/^(AS|AK|AP|AR) Battery System$/i.test(clean(category));
    var categories=seriesPage ? distinct(data.families.map(function(f){return f.category;})) : [];
    ...
    var categoryButtons=categories.map(function(x){
      return '<button type="button" data-filter-category="'+esc(x)+'">'+esc(x)+'</button>';
    }).join('');
    var subtypeButtons=seriesPage ? categories.map(function(parent){
      var children=distinct(data.families.filter(function(f){return f.category===parent;})
        .map(function(f){return f.subcategory;}));
      if(children.length<2) return '';
      if(parent.toLowerCase()==='blowers'){
        children.sort(function(a,b){return (/handheld/i.test(a)?0:/backpack/i.test(a)?1:2)-(/handheld/i.test(b)?0:/backpack/i.test(b)?1:2);});
      }
      if(parent.toLowerCase()==='hedge trimmers'){
        children.sort(function(a,b){return (/^hedge trimmer$/i.test(a)?0:1)-(/^hedge trimmer$/i.test(b)?0:1);});
      }
      return children.map(function(x){
        var label=parent.toLowerCase()==='blowers' ? x.replace(/\bBlower\b/i,'').trim()||x : x;
        return '<button type="button" data-filter-type="'+esc(x)+'" data-parent-category="'+esc(parent)+'" hidden>'+esc(label)+'</button>';
      }).join('');
    }).join('') : '';
    var typeButtons=seriesPage ? categoryButtons : types.map(function(x){
    ...
    return '<section id="wep-smart-catalog" class="wep-smart-catalog">'+
      ...
      '<div class="wep-smart-toolbar">'+
        '<div><strong>Type</strong><div class="wep-filter-buttons" id="wep-type-filters">'+typeButtons+'</div>'+
          (subtypeButtons ? '<div id="wep-subtype-filters" hidden><strong>Subtype</strong><div class="wep-filter-buttons">'+subtypeButtons+'</div></div>' : '')+'</div>'+
  ...
  function smartCss(){
    ...
    '.wep-smart-card[hidden],.wep-filter-buttons button[hidden],#wep-subtype-filters[hidden]{display:none!important}'+
  ...
  function runtimeScript(data){
    ...
    return '<script>(function(){'+
      'var DATA='+safe+';var families=DATA.families||[];var byKey={};families.forEach(function(f){byKey[f.key]=f;});var cart=[];var compare=[];var categoryFilter="";var type="";var power="";var series="";'+
      ...
      'function reTotal(key){var v=selectedVariant(key);if(!v)return;var cat=encodeURIComponent((byKey[key]&&byKey[key].category)||"Equipment"),ret=encodeURIComponent(location.href);var o=q("[data-options=\\\""+CSS.escape(key)+"\\\"]");if(o)o.href=(/^(localhost|127\\.0\\.0\\.1)$/.test(location.hostname)?"product-options.html":"https://www.westendpower.com/stihl-equipment-configurator/product-options.html")+"?sku="+encodeURIComponent(v.sku)+"&category="+cat+"&return="+ret;var r=q("[data-runtime=\\\""+CSS.escape(key)+"\\\"]");if(r)r.href=v.configure||"#";var link=q("[data-product-link=\\\""+CSS.escape(key)+"\\\"]");if(link){var url=String(v.details||"");if(/^https?:\\/\\//i.test(url))link.href=url;else link.removeAttribute("href")}}'+
      'function filters(){var search=(q("#wep-smart-search")||{}).value||"";search=search.toUpperCase();var only=!!(q("#wep-stock-only")||{}).checked;var visible=0;qa(".wep-smart-card").forEach(function(card){var f=byKey[card.dataset.family];var ok=(!categoryFilter||f.category===categoryFilter)&&(!type||f.subcategory===type)&&(!power||f.power===power)&&(!series||f.series===series)&&(!only||f.stock>0||f.order>0)&&(!search||(f.name+" "+f.subcategory+" "+f.power+" "+f.series).toUpperCase().indexOf(search)>=0);card.hidden=!ok;if(ok)visible++;});var n=q("#wep-result-count");if(n)n.textContent=visible}'+
      ...
      'qa("[data-filter-category]").forEach(function(b){b.onclick=function(){var value=b.dataset.filterCategory||"";categoryFilter=categoryFilter===value?"":value;type="";qa("[data-filter-category]").forEach(function(x){x.classList.toggle("active",categoryFilter!==""&&x.dataset.filterCategory===categoryFilter)});qa("[data-parent-category]").forEach(function(x){x.hidden=x.dataset.parentCategory!==categoryFilter;x.classList.remove("active")});var panel=q("#wep-subtype-filters");if(panel)panel.hidden=!categoryFilter||!qa("[data-parent-category]").some(function(x){return !x.hidden});filters()}});'+
