(function(){
  ...
  function pageData(products){
    ...
    var chargers=activeList(liveState && liveState.chargers)
      ...
      .filter(function(x){return x.price>0;});
    var accessories=activeList(liveState && liveState.accessories)
      .map(function(x){return optionPayload(x,'accessory');})
      .filter(function(x){return x.price>0;});
    var attachments=activeList(liveState && liveState.attachments)
      .map(function(x){return optionPayload(x,'attachment');})
      .filter(function(x){return x.price>0;});
    var parts=activeList(liveState && liveState.parts)
      .map(function(x){return optionPayload(x,'part');})
      .filter(function(x){return x.price>0;});
    return {
      families:families,
      batteries:batteries,
  ...
  function smartCss(){
    ...
    '.wep-add-cart{min-height:36px!important;font-size:13px!important;font-weight:800!important}'+
    '.wep-component-buy{display:flex;align-items:center;gap:7px;flex-wrap:wrap;margin-top:8px}'+
    '.wep-component-buy input{width:64px;height:34px;padding:5px 7px;border:1px solid #aaa;border-radius:6px;font-weight:800}'+
    '.wep-component-buy button{min-height:34px;padding:6px 10px;border:1px solid #222;border-radius:6px;background:#222;color:#fff;font-weight:900;cursor:pointer}'+
    '.wep-component-buy small{flex-basis:100%;color:#555;font-size:11px}'+'</style>';
  }
  function runtimeScript(data){
    var safe=JSON.stringify(data).replace(/</g,'\\u003c');
    return '<script>(function(){'+
      'var DATA='+safe+';var families=DATA.families||[];var byKey={};families.forEach(function(f){byKey[f.key]=f;});var cart=[];var compare=[];var type="";var power="";var series="";var DEALERSPIKE_PRODUCT_BY_SKU={"EA10 430 5502":58};'+
      ...
      'function normText(v){return String(v==null?"":v).toUpperCase().replace(/[^A-Z0-9]+/g,"")}'+
      'function componentPool(){return [].concat(DATA.batteries||[],DATA.chargers||[],DATA.accessories||[],DATA.attachments||[],DATA.parts||[])}'+
      'function componentNames(x){return [x.label,x.description,x.model,x.batteryId,x.chargerId,x.chargerName,x.sku,x.stihlId].filter(Boolean).map(normText).filter(Boolean)}'+
      'function findComponentForHeading(text){var n=normText(text);if(!n)return null;var best=null,bestLen=0;componentPool().forEach(function(x){componentNames(x).forEach(function(k){if(k.length>=4&&(n.indexOf(k)>=0||k.indexOf(n)>=0)&&k.length>bestLen){best=x;bestLen=k.length}})});return best}'+
      'function dealerSpikeGo(sku,qty){var webstoreId=DEALERSPIKE_PRODUCT_BY_SKU[String(sku||"").trim()];if(!webstoreId){alert("Online checkout setup is not complete for this item yet.");return false}try{sessionStorage.setItem("wepDealerSpikeRequestedQty",String(Math.max(1,Number(qty)||1)));sessionStorage.setItem("wepDealerSpikeRequestedSku",String(sku||""))}catch(e){}window.location.href="https://www.westendpower.com/ecommerce/products/view/"+encodeURIComponent(webstoreId);return true}'+
      'function enhanceComponentListings(){var mapped=componentPool().filter(function(x){return DEALERSPIKE_PRODUCT_BY_SKU[String(x.sku||"").trim()]});if(!mapped.length)return;qa("h3,h4,strong").forEach(function(h){if(h.closest(".wep-smart-card")||h.closest(".wep-component-buy"))return;var item=findComponentForHeading(h.textContent||"");if(!item||!DEALERSPIKE_PRODUCT_BY_SKU[String(item.sku||"").trim()])return;var host=h.parentElement;if(!host||host.querySelector(".wep-component-buy"))return;var wrap=document.createElement("div");wrap.className="wep-component-buy";wrap.innerHTML="<label>Qty <input type=\\\"number\\\" min=\\\"1\\\" max=\\\"99\\\" value=\\\"1\\\" aria-label=\\\"Quantity\\\"></label><button type=\\\"button\\\">Add to Cart</button><small>Product availability will be confirmed before your order is processed.</small>";var input=wrap.querySelector("input"),button=wrap.querySelector("button");button.addEventListener("click",function(){dealerSpikeGo(item.sku,input.value)});host.appendChild(wrap)})}'+
