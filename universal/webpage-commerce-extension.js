(function(){
  ...
  function renderFamilyCard(f,data){
    var first=f.variants[0]||{};
    var activeSales=(f.variants||[]).map(function(v){return {variant:v,sale:saleInfo(v)};}).filter(function(x){return !!x.sale;});
    ...
    return '<article class="wep-smart-card" data-family="'+esc(f.key)+'" data-type="'+esc(f.subcategory)+'" data-power="'+esc(f.power)+'" data-stock="'+(f.stock>0?'1':'0')+'" data-order="'+(f.order>0?'1':'0')+'">'+
      ...
      '<div class="wep-smart-media">'+image+promoOverlay+'<span class="wep-availability-badge">'+esc(stockText(f))+'</span></div>'+
