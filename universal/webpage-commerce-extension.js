(function(){
  ...
  function runtimeScript(data){
    ...
    return '<script>(function(){'+
      ...
      'function reTotal(key){var v=selectedVariant(key);if(!v)return;var e=selectedExtras(key),t=Number(v.price||0)+Number(e.battery&&e.battery.price||0)+Number(e.charger&&e.charger.price||0);var el=q("[data-total=\\\""+CSS.escape(key)+"\\\"]");if(el)el.textContent="Selected total: "+m(t);var a=q("[data-configure=\\\""+CSS.escape(key)+"\\\"]");if(a)a.href=v.configure||"#";var o=q("[data-options=\\\""+CSS.escape(key)+"\\\"]");if(o)o.href="product-options.html?sku="+encodeURIComponent(v.sku)+"&category="+encodeURIComponent((byKey[key]&&byKey[key].category)||"Equipment")+"&return="+encodeURIComponent(location.href)}'+
      ...
      'function drawCart(){var lines=q("#wep-cart-lines"),count=q("#wep-cart-count"),total=q("#wep-cart-total");if(!lines)return;if(!cart.length){lines.innerHTML="<p class=\\\"wep-cart-empty\\\">Your cart is empty.</p>"}else{lines.innerHTML=cart.map(function(x,i){return "<div class=\\\"wep-cart-line\\\"><div><strong>"+esc(x.name)+" — "+esc(x.variant.label)+"</strong><small>Qty "+(x.qty||1)+( [x.battery&&x.battery.label,x.charger&&x.charger.label].filter(Boolean).length?" · ":"")+[x.battery&&x.battery.label,x.charger&&x.charger.label].filter(Boolean).map(esc).join(" · ")+"</small></div><strong>"+m(x.total)+"</strong><div><a href=\\\""+esc(x.variant.configure)+"\\\" target=\\\"_blank\\\" rel=\\\"noopener\\\">Build &amp; Price</a> · <button type=\\\"button\\\" data-remove-cart=\\\""+i+"\\\">Remove</button></div></div>"}).join("")}if(count)count.textContent=cart.length+" item"+(cart.length===1?"":"s");if(total)total.textContent=m(cart.reduce(function(s,x){return s+x.total},0));qa("[data-remove-cart]").forEach(function(b){b.onclick=function(){cart.splice(Number(b.dataset.removeCart),1);drawCart()}})}'+
      'function addCart(key){var f=byKey[key],v=selectedVariant(key);if(!f||!v)return;var e=selectedExtras(key),qel=q("[data-main-qty=\\\""+CSS.escape(key)+"\\\"]"),qty=Math.max(1,Number(qel&&qel.value)||1),unit=Number(v.price||0)+Number(e.battery&&e.battery.price||0)+Number(e.charger&&e.charger.price||0),t=unit*qty;cart.push({name:f.name,variant:v,battery:e.battery,charger:e.charger,qty:qty,unit:unit,total:t});drawCart();q("#wep-cart").scrollIntoView({behavior:"smooth",block:"start"})}'+
