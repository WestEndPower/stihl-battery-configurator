(function(){
  ...
  function qty(v){var n=Number(v);return Number.isInteger(n)&&n>0&&n<=25?n:0;}
  ...
  DATA.families.forEach(function(f){
    (f.items||[]).forEach(function(p){
      products.set(String(p.SKU||"").trim().toUpperCase(),p);
    });
  });
  (DATA.batteries||[]).forEach(function(p){
    products.set(String(p.sku||"").trim().toUpperCase(),{
      SKU:p.sku,Description:p.label,Model:p.label,MSRP:p.price,
      Active:"T",BuyOnlineEligible:"T",ShippingEligible:"F"
    });
  });
  (DATA.chargers||[]).forEach(function(p){
    products.set(String(p.sku||"").trim().toUpperCase(),{
      SKU:p.sku,Description:p.label,Model:p.label,MSRP:p.price,
      Active:"T",BuyOnlineEligible:"T",ShippingEligible:"F"
    });
  ...
  function eligible(p){
    return !!p &&
      String(p.BuyOnlineEligible||"").trim().toUpperCase()==="T" &&
      String(p.Active||"").trim().toUpperCase()==="T" &&
      Number(p.SalePrice||p.MSRP||0)>0;
  }
  ...
  note.textContent=TEST?"TEST MODE: Stripe test checkout":"Secure online checkout";
  ...
  function add(sku){
    ...
    if(existing){if(existing.quantity>=25)return showStatus("Maximum quantity is 25.");existing.quantity++;}
  ...
  document.querySelectorAll("[data-component-cart]").forEach(function(button){
    var sku=String(button.dataset.componentCart||"").trim();
    if(!sku)return;
    var replacement=button.cloneNode(true);
    replacement.textContent="Add to Cart";
    replacement.addEventListener("click",function(event){
      event.preventDefault();
      event.stopPropagation();
      var qtyInput=document.querySelector("[data-component-qty=\\\""+CSS.escape(sku)+"\\\"]");
      var count=qty(qtyInput?qtyInput.value:1)||1;
      for(var i=0;i<count;i++)add(sku);
    });
  ...
  function render(){
    ...
    lines.forEach(function(entry){
      ...
      var input=el("input");input.type="number";input.min="1";input.max="25";
    ...
    form.onsubmit=async function(event){
      ...
      try{
        var response=await fetch(API+"/online-order",{
          method:"POST",headers:{"Content-Type":"application/json"},
          body:JSON.stringify({customer:customer,fulfillment:fulfillment.value,
            cartItems:read().map(function(line){return {sku:line.sku,quantity:line.quantity};}),
            termsAccepted:true})
        });
