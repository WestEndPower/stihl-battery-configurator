(function(){
  ...
  function enrichPackageVariants(families,liveState){
    ...
    var compatByTool=new Map(compatibility.filter(function(row){
      return !clean(row.Active) || isTrue(row.Active);
    }).map(function(row){return [norm(row.ToolSKU),row];}));
        ...
        if(compat){
          ...
          if(battery && charger){
            ...
            family.variants.push({
              sku:toolVariant.sku,label:'Recommended Package',description:toolVariant.description,
  ...
  function renderFamilyPrices(f){
    var tool=f.variants.find(function(v){return !v.isKit && !v.isRecommendedPackage;});
    ...
    if(tool){
      ...
      out+='<div class="wep-price-choice wep-tool-choice"><span class="wep-choice-kicker">'+(toolSale?'Save '+dollar+toolSale.savings.toFixed(2)+(toolSale.end?' thru '+esc(shortDate(toolSale.end)):''):'TOOL ONLY')+'</span><p class="wep-price-pair">'+
    ...
    if(kit){
      ...
      out+='<div class="wep-price-choice wep-kit-choice"><span class="wep-choice-kicker">'+(kitSale?'Save '+dollar+kitSale.savings.toFixed(2)+(kitSale.end?' thru '+esc(shortDate(kitSale.end)):''):'PACKAGE PRICING')+'</span><p class="wep-price-pair">'+
  ...
  function renderFamilyCard(f,data){
    ...
    return '<article class="wep-smart-card" data-family="'+esc(f.key)+'" data-type="'+esc(f.subcategory)+'" data-power="'+esc(f.power)+'" data-stock="'+(f.stock>0?'1':'0')+'" data-order="'+(f.order>0?'1':'0')+'">'+
      ...
      '<div class="wep-smart-media">'+image+'<span class="wep-availability-badge">'+esc(stockNote(f))+'</span></div>'+
      '<div class="wep-smart-body">'+
        ...
        '</div>'+
        '<a class="wep-product-link" data-product-link="'+esc(f.key)+'"'+((first.details && /^https?:\/\//i.test(first.details)) ? ' href="'+esc(first.details)+'"' : ' hidden')+' target="_blank" rel="noopener noreferrer">View on West End Power</a>'+
      '</div>'+
    '</article>';
  }
  ...
  function smartCss(){
    ...
    '.wep-smart-card{overflow:hidden;transition:transform .18s ease,box-shadow .18s ease}.wep-smart-card:hover{transform:translateY(-3px);box-shadow:0 12px 28px rgba(0,0,0,.14)}.wep-smart-body h3{font-size:25px;letter-spacing:-.02em}.wep-smart-actions a,.wep-smart-actions button{font-weight:900}.wep-smart-actions a:first-child{background:#c8102e;color:#fff;border-color:#c8102e}.wep-availability-badge{border:2px solid rgba(255,255,255,.85)}.wep-smart-media{margin:12px 12px 0;border:1px solid #ddd;border-radius:10px;overflow:hidden}.wep-smart-body .wep-smart-stock{margin:0 0 8px;color:#287a32;font-size:14px}.wep-price-choice .wep-price-pair{display:flex;flex-wrap:wrap;align-items:baseline;column-gap:12px;row-gap:4px}.wep-price-choice .wep-price-pair del{color:#666;white-space:nowrap}.wep-price-choice .wep-price-pair strong{white-space:nowrap}.wep-price-choice:nth-child(2) .wep-price-pair{justify-content:flex-end}.wep-choice-kicker{font-size:11px;white-space:nowrap;letter-spacing:0}.wep-product-link{display:block;margin-top:12px;color:#b20a28;font-weight:800;text-decoration:underline}@media(max-width:760px){.wep-price-lines{grid-template-columns:1fr}.wep-price-choice:nth-child(2){text-align:left}.wep-price-choice:nth-child(2) p{text-align:left}.wep-price-choice:nth-child(2) .wep-price-pair{justify-content:flex-start}.wep-smart-toolbar{grid-template-columns:1fr}.wep-smart-grid{grid-template-columns:1fr}.wep-smart-option-row{grid-template-columns:1fr}.wep-smart-actions{grid-template-columns:1fr}.wep-cart-line{grid-template-columns:1fr auto}.wep-cart-line>a{grid-column:1/-1}.wep-cart-footer{display:block}.wep-compare-bar{margin-left:8px;margin-right:8px}}'+
    '</style>';
  }
  function runtimeScript(data){
    ...
    return '<script>(function(){'+
      ...
      'function reTotal(key){var v=selectedVariant(key);if(!v)return;var cat=encodeURIComponent((byKey[key]&&byKey[key].category)||"Equipment"),ret=encodeURIComponent(location.href);var o=q("[data-options=\\\""+CSS.escape(key)+"\\\"]");if(o)o.href="product-options.html?sku="+encodeURIComponent(v.sku)+"&category="+cat+"&return="+ret;var r=q("[data-runtime=\\\""+CSS.escape(key)+"\\\"]");if(r)r.href=v.configure||"#";var link=q("[data-product-link=\\\""+CSS.escape(key)+"\\\"]");if(link){var url=String(v.details||"");link.hidden=!/^https?:\\/\\//i.test(url);if(!link.hidden)link.href=url;else link.removeAttribute("href")}}'+
      ...
      'function drawCart(){var lines=q("#wep-cart-lines"),count=q("#wep-cart-count"),total=q("#wep-cart-total");if(!lines)return;if(!cart.length){lines.innerHTML="<p class=\\\"wep-cart-empty\\\">Your cart is empty.</p>"}else{lines.innerHTML=cart.map(function(x,i){return "<div class=\\\"wep-cart-line\\\"><div><strong>"+esc(x.name)+" — "+esc(x.variant.label)+"</strong><small>Qty "+(x.qty||1)+(x.variant.packageIncludes?" · Includes "+esc(x.variant.packageIncludes):"")+"</small></div><strong>"+m(x.total)+"</strong><div><button type=\\\"button\\\" data-remove-cart=\\\""+i+"\\\">Remove</button></div></div>"}).join("")}if(count)count.textContent=cart.length+" item"+(cart.length===1?"":"s");if(total)total.textContent=m(cart.reduce(function(s,x){return s+x.total},0));qa("[data-remove-cart]").forEach(function(b){b.onclick=function(){cart.splice(Number(b.dataset.removeCart),1);drawCart()}})}'+
